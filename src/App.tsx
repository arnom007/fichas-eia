...
export default function App() {
  ...
  const processTextData = (text: string) => {
    // 1. GLOBAL CLEANUP 🧹 (Sua lógica de limpeza inicial perfeitamente mantida)
    ...
    // 3. TABLE EXTRACTION WITH NEW TOKENIZATION 🧩 (O seu motor genial)
    ...
    // The new, ultra-permissive regex that isolates Phases and Grades perfectly
    ...
    for (let i = 0; i < parts.length - 1; i += 2) {
      ...
      if (numMatches.length > 0) {
        ...
        for (let j = 0; j < numMatches.length; j++) {
            ...
            // Clean weird leftover punctuation
    ...
    if (idxComentarios !== -1) {
      ...
      // New Regex that handles missing parens and missing colons
          ...
          for (const line of lines) {
            ...
            for (let j = 0; j < line.length; j++) {
              ...
              if (j < line.length - 1) {
                ...
                // SE O BURACO FÍSICO FOR GRANDE (COLUNA), INSERE QUEBRA DE LINHA (Proteção Mestre)
                if (gap > 35) {
                  lineStr += ' \n ';
                } else {
